// ============================================================================
// GHOSTTY SHADER: STABLE OKLAB BLOOM (WITHOUT CURSOR EFFECTS)
// ============================================================================
...
float f(float x) {
    if (x >= 0.0031308) {
        return 1.155 * pow(x, 1.0 / 2.4) - 0.055;
    } else {
        return 12.92 * x;
    }
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // CONVERT SCENE TO OKLAB & APPLY STABLE BLOOM
    vec4 source = toOklab(baseColor);
    vec4 dest = source;

    if (source.x > DIM_CUTOFF) {
        dest.x *= 1.15; // Helderheidsboost voor oplichtende elementen
    } else {
        vec2 stepSize = vec2(2.414) / iResolution.xy;
        vec3 glow = vec3(0.0);
        ...
        for (int i = 0; i < 24; i++) {
            ...
            // Sample omliggende pixels in Oklab space
            vec4 c = toOklab(texture(iChannel0, uv + s.xy * stepSize));

            if (c.x > DIM_CUTOFF) {
                glow.yz += c.yz * weight * 0.20;
